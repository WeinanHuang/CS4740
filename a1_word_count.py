...
# create a n*n matrix to store prob for all n words

wd_base = list(set(Text.split(' ')))
print len(wd_base)

wd_freq = collections.Counter(Text.split(' '))
#print wd_freq
...
for wd in wd_base[:100]:
    mat[wd] = {}
    wdFreq = max(Text.split(' ').count(wd),1)
    for wd1 in wd_base[:100]:
        biGram = wd + ' ' + wd1
        mat[wd][wd1] = 1.* Text.count(biGram) / wdFreq
'''
